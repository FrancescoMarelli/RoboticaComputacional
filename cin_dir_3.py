...
def muestra_robot(O,ef=[]):
  # Pinta en 3D
  OR = ramal(O) #crea secuencia de puntos en lista para cada eje
  OT = np.array(OR).T
  fig = plt.figure()
  ...
  for xb, yb, zb in zip(Xb, Yb, Zb): #da color a los tres origenes
     ax.plot([xb], [yb], [zb], 'w')
  ...
  if isinstance(O[0],list):
    for i in range(len(O)):
      if isinstance(O[i][0],list):
        ...
        print('(O'+str(i+base)+sufijo+')0\t= '+str([round(j,3) for j in O[i]])) #redondea 3?
  else:
...
p=[float(i) for i in sys.argv[1:nvar+1]] #Almacena valor articulaciones 

# Parámetros D-H:
#        1      2       2'     3      4    5.1    5.2
d  = [p[0],     0,      0,    0,       0,       0,       0,    0]  
th = [   0,  p[1],      0,   90,  90+p[4], 90-p[5], 90+p[6],  90]
a  = [   0,     5,   p[2],    0,       0,       2,       2,    2]
...
muestra_robot   ([o00,o10,o20,o30,o40,o50,o60,o70])
input()
