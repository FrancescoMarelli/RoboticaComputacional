...
class robot:
  def __init__(self):
    ...
    self.y             = 0.
    self.orientation   = 0.
    self.forward_noise = 0. # errores, ruido de avance y ruido de giro
    self.turn_noise    = 0.
    self.sense_noise   = 0. # ruido de sensado, cuando calculamos distancia entre robot y balizas
    self.weight        = 1.
    self.old_weight    = 1.
    self.size          = 1.
  ...
  def pose(self):
    # Obtener pose actual desde se piensa que robot esta, nunca sabemos donde esta 
  ...
  def sense(self, landmarks):
    # Calcular las distancias a cada una de las balizas entre punto robot y punto baliza
  ...
  def move_triciclo(self, turn, forward, largo):
    # Modificar pose del robot (Ackermann) usaremos el move no este metodo
    dist = float(forward) + random.gauss(0., self.forward_noise)
  ...
  def measurement_prob(self, measurements, landmarks):
    # Calcular la probabilidad de una medida. compara medidas con robot ideal y robot real
    self.weight = 0.
    n=0
    ...
    return self.weight # si diferencia grande es que se desvió
